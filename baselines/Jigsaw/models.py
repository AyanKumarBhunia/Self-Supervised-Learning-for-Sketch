...
from utils import Jigsaw_Utility
...
class Sketch_Classification(nn.Module):
    def __init__(self, hp):
        ...
        self.hp = hp
        self.step = 0
        self.jigsaw_handler = Jigsaw_Utility()
    ...
    def train_jigsaw_self_supervised(self, batch, step):
        self.train()

        batch_image = batch['sketch_img'].to(device)
        batch_image_shuffle, perms = self.jigsaw_handler.permute_patch(batch_image)
        # batch_image_restored = self.jigsaw_handler.restore_patch(batch_image_shuffle, perms)
        # torchvision.utils.save_image(batch_image_shuffle, 'shuffle.jpg')
        # torchvision.utils.save_image(batch_image_restored, 'retored.jpg')
        y_label  = self.jigsaw_handler.perm2vecmat(perms)
        # perms_back = self.jigsaw_handler.vecmat2perm(y_label)
        # assert self.jigsaw_handler.vecmat2perm(y_label).equal(perms)



        self.step = step
        self.optimizer.zero_grad()
        output = self.Network(batch_image_shuffle.to(device))

        output = self.jigsaw_handler.my_sinkhorn(output)

        loss = F.binary_cross_entropy(output, y_label, reduction='none').sum(1).mean()
        loss.backward()
        self.optimizer.step()

        if step%200 == 0:
            batch_jigsaw_instance_accuracy, patch_level_acc = \
            self.jigsaw_handler.compute_acc(self.jigsaw_handler.vecmat2perm(output), perms, True)
            print('Batch Jigsaw Instance Accuracy %- {}, Batch Jigsaw Patchwise Accuracy: {}'.format(batch_jigsaw_instance_accuracy, patch_level_acc))
